...
class generate_target():
  def __init__(self):
#input==============================================================================================  
    self.grasping_point_sub = rospy.Subscriber("/grasping_target_point", Point32, self.callback_grasping_point)
    self.handle_point_sub = rospy.Subscriber("/handle_target_point", Point32, self.callback_handle_point)
    self.cam_in_base_tf_sub = rospy.Subscriber("/cam_in_base_tf", Pose, self.callback_cam_in_base)
#output=======================================================================================      
    self.ee_goal_in_base_pub = rospy.Publisher("/ee_place_target_in_base", PoseStamped, queue_size=1)   
#member===========================================================================================      
  ...
  def callback_grasping_point(self, data):
    ...
    if self.if_recieved_handle == True and self.if_recieved_grasping == True:
      ...
      # ee target
      ee_target_pose = give_PoseStamped(posestamped_crr.header.frame_id, q_new, self.grasping_point)
...
def main(args):
  rospy.init_node("generate_place_target", anonymous=True)
  gt = generate_target()
