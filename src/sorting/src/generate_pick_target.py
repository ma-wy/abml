...
#member===========================================================================================      
    ...
    self.cam_in_base_tf = []
    self.bias = array([0, 0, 0.024])
    # z: -0.08965410419036911
    #
  ...
  def callback_grasping_point(self, data):
    ...
    grasping_point = deepcopy(self.grasping_point)
    grasping_point[-1] = self.handle_point[-1]
    
    if self.if_recieved_handle == True and self.if_recieved_grasping == True:
      # align x axis with the instrument
      x_axis_new = (handle_point - grasping_point)/norm(handle_point - grasping_point)
      ...
      # ee target
      ee_target_pose = give_PoseStamped(posestamped_crr.header.frame_id, q_new, grasping_point)
...
def main(args):
  rospy.init_node("generate_pick_target", anonymous=True)
  gt = generate_target()
