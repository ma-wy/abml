...
# members
  ...
  def callback_hand(self, data):
    ...
    # hand frame
    # Y axis 
    # 1:thumb_tip + 2:index_tip - 0:wrist
    y_axis = array(hand_pc[0][0:3]) - (array(hand_pc[1][0:3]) + array(hand_pc[2][0:3]))
    # palm: 0:wrist 6:index_mcp 9:pinky_mcp
    z_axis = cross(array(hand_pc[9][0:3]) - array(hand_pc[0][0:3]), array(hand_pc[6][0:3]) - array(hand_pc[0][0:3]))
    ...
    hand_frame = give_PoseStamped('base', q, hand_pc[-1][0:3])
    self.hand_frame_pub.publish(hand_frame) 
