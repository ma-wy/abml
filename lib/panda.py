...
class Panda():
    def __init__(self):
        ...
        self.curr_ori=None
        self.curr_q = None
        self.attractor_distance_threshold=0.0001 # if the attractor distance is larger than the treshold, the robot will wait
        ...
        self.move_command.goal.speed=0.5
        self.grasp_command.goal.epsilon.inner = 0.3
        self.grasp_command.goal.epsilon.outer = 0.3
        self.grasp_command.goal.speed = 0.5
        self.grasp_command.goal.force = 2
        self.grasp_command.goal.width = 1
        self.reset_ee_p = np.array([0.35, 0, 0.4])
        self.reset_ee_q = list_2_quaternion([0, 1.0, 0, 0])
        self.reset_ee_pose = array_quat_2_pose(self.reset_ee_p, self.reset_ee_q)
    ...
    def ee_pos_callback(self, curr_conf):
        ...
        self.curr_q = np.array([curr_conf.pose.orientation.x, curr_conf.pose.orientation.y, curr_conf.pose.orientation.z, curr_conf.pose.orientation.w])
    ...
    def home(self):
        ...
        goal = self.reset_ee_pose#array_quat_2_pose(pos_array, quat)
        goal.header.seq = 1
    ...
    def go_to_pose(self, goal_pose, interp_dist=0.005, interp_dist_polar=0.02): ##### Are both interpolation distances needed?
        # the goal pose should be of type PoseStamped. E.g. goal_pose=PoseStampled()
        r = rospy.Rate(100)
        start = self.curr_pos
        start_ori = self.curr_ori
        goal_array = np.array([goal_pose.position.x, goal_pose.position.y, goal_pose.position.z])
        ...
        q_goal=np.quaternion(goal_pose.orientation.w, goal_pose.orientation.x, goal_pose.orientation.y, goal_pose.orientation.z)
        ...
        inner_prod=q_start.x*q_goal.x+q_start.y*q_goal.y+q_start.z*q_goal.z+q_start.w*q_goal.w
        #print(inner_prod)
        theta= np.arccos(np.abs(inner_prod)) - np.pi/2.0
        step_num_polar = math.floor(theta / interp_dist_polar)
        
        print()
        print(theta)
        print(step_num_polar)
        print(dist)
        print(step_num_lin)
        
        step_num=np.max([abs(step_num_polar),abs(step_num_lin)])
        
        x = np.linspace(start[0], goal_pose.position.x, step_num)
        y = np.linspace(start[1], goal_pose.position.y, step_num)
        z = np.linspace(start[2], goal_pose.position.z, step_num)
        print(start_ori)
        print(goal_pose.orientation)
        q_w = np.linspace(start_ori[0], goal_pose.orientation.w, step_num)
        q_x = np.linspace(start_ori[1], goal_pose.orientation.x, step_num)
        q_y = np.linspace(start_ori[2], goal_pose.orientation.y, step_num)
        q_z = np.linspace(start_ori[3], goal_pose.orientation.z, step_num)
        print(q_w)
        print(q_x)
        print(q_y)
        print(q_z)
        ...
        while i  < 10:#step_num:
            ...
            quat = np.array([q_w[i], q_x[i], q_y[i], q_z[i]])
